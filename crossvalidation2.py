...
test_json_path = "C:\\work\\CIMC_Data\\COCO_CIMC_RR\\annotations\\instances_test2017.json"
test_images_path = "C:\\work\\CIMC_Data\\COCO_CIMC_RR\\test2017"
...
register_coco_instances("test_set", {}, test_json_path, test_images_path)
...
for fold, (train_idx, val_idx) in enumerate(splits):
    ...
    cfg.DATASETS.TEST = (val_ds_name, "test_set")  # Include the original test set here
    ...
    # Evaluate the model on the validation set and the original test set
    evaluator_val = COCOEvaluator(val_ds_name, cfg, False, output_folder=os.path.join(cfg.OUTPUT_DIR, "coco_eval_val"))
    val_loader = build_detection_test_loader(cfg, val_ds_name)
    metrics_val = trainer.test(cfg, trainer.model, evaluators=[evaluator_val])

    evaluator_test = COCOEvaluator("test_set", cfg, False, output_folder=os.path.join(cfg.OUTPUT_DIR, "coco_eval_test"))
    test_loader = build_detection_test_loader(cfg, "test_set")
    metrics_test = trainer.test(cfg, trainer.model, evaluators=[evaluator_test])

    results.append({"fold": fold, "validation_metrics": metrics_val, "test_metrics": metrics_test})
